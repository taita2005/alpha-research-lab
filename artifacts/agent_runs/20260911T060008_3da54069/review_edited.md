...
The volume filter retains assets whose short-term average dollar volume exceeds its long-term average. Dates with fewer than five eligible assets do not receive an IC estimate. As a result, its IC is measured on a distinct, filtered subset of assets and dates, making direct IC comparisons against full-coverage signals improper without controlling for sample differences [methodology].
...
2. **Asset Independence**: Overlapping ETF exposures limit cross-sectional diversity and complicate statistical inference [methodology].
...
The registered strategy and research artifacts remained unchanged during this review run. Future strategy revisions must not treat the already-observed holdout as an untouched test set. [methodology] Recommended future research steps include:
